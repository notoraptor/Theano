...
from theano.tensor.nnet.tests.test_spatialtf import TestTransformer
...
class TestDnnTransformer(TestTransformer):
    ...
    def run_gpu_vs_cpu(self, case_index):
        ...
        atol = rtol
        try:
            utt.assert_allclose(out_gpu, out_cpu, atol=atol, rtol=rtol)
    ...
    def run_gpu_vs_cpu_grad_grid(self, case_index):
        t_inp = T.tensor4('inp')
        t_theta = T.tensor3('theta')
        t_out_dims = T.lvector('out_dims')
        ...
        cpu_function = theano.function([t_inp, t_theta, t_out_dims], t_grad_scalar, mode=mode_without_gpu)
        gpu_function = theano.function([t_inp, t_theta, t_out_dims], t_grad_scalar, mode=mode_with_gpu)
        self.assert_cpu_function(cpu_function)
        self.assert_gpu_function(gpu_function)

        inp_shape = self.inp_cases[case_index]
        transformation = self.transform_cases[case_index]
        inp, theta, scale_width, scale_height = self.getInputs(inp_shape, transformation)
        out_dims = [int(val) for val in tuple(inp_shape[:2]) + (np.ceil(inp_shape[2] * scale_height),
                                                                np.ceil(inp_shape[3] * scale_width))]
        cpu_out = cpu_function(inp, theta, out_dims)
        gpu_out = gpu_function(inp, theta, out_dims)
        try:
            utt.assert_allclose(np.asarray(gpu_out), cpu_out)
        except Exception as e:
            print('Failing case:')
            print('Input shape:', inp_shape)
            print('Ttransform:')
    ...
    def run_gpu_vs_cpu_grad_input(self, case_index):
        # Setup symbolic variables
        t_inp = T.tensor4('inp')
        t_theta = T.tensor3('theta')
        t_scale_height = T.scalar('scale_height')
    ...
    def run_gpu_vs_cpu_grad_theta(self, case_index):
        # Setup symbolic variables
        t_inp = T.tensor4('inp')
        t_theta = T.tensor3('theta')
        t_scale_height = T.scalar('scale_height')
    ...
    def test_gpu_vs_cpu(self):
        for case_index in range(len(self.inp_cases)):
            yield (self.run_gpu_vs_cpu, case_index)

    def test_gpu_vs_cpu_grad_grid(self):
        for case_index in range(len(self.inp_cases)):
            yield (self.run_gpu_vs_cpu_grad_grid, case_index)

    def test_gpu_vs_cpu_grad_input(self):
        for case_index in range(len(self.inp_cases)):
            yield (self.run_gpu_vs_cpu_grad_input, case_index)

    def test_gpu_vs_cpu_grad_theta(self):
        for case_index in range(len(self.inp_cases)):
            yield (self.run_gpu_vs_cpu_grad_theta, case_index)
    ...
    def test_simple_case(self):
        ...
        st = spatialtf(t_img, t_theta, scale_height=scale_height, scale_width=scale_width)
        st_func = theano.function([t_img, t_theta], st, mode=mode_with_gpu)
        # Check if function graph contains the spatial transformer's grid and sampler Ops
        apply_nodes = st_func.maker.fgraph.apply_nodes
        assert any([isinstance(node.op, self.transformer_grid_op) for node in apply_nodes])
        assert any([isinstance(node.op, self.transformer_sampler_op) for node in apply_nodes])

        img_out_gpu = np.asarray(st_func(img, theta))

        # Setup CPU Op
        st_cpu = spatialtf(t_img, t_theta, scale_width, scale_height)
